...
const schema = yup.object({
  nome:     yup.string().trim().min(3, 'Nome deve ter ao menos 3 caracteres').required('Nome é obrigatório'),
  telefone: yup.string().matches(/^\(\d{2}\) \d{4,5}-\d{4}$/, 'Informe um telefone válido — ex: (99) 99999-9999').required('Telefone é obrigatório'),
  ...
  lgpd:        yup.bool().oneOf([true], 'Você precisa aceitar os termos da LGPD').required(),
  regulamento: yup.bool().oneOf([true], 'Você precisa confirmar que leu o regulamento').required(),
})
...
async function getRecaptchaToken() {
  ...
  return new Promise(resolve => {
    window.grecaptcha.ready(async () => {
      try { resolve(await window.grecaptcha.execute(SITE_KEY, { action: 'inscricao_v2' })) }
      catch { resolve('') }
    })
  })
}
...
export default function PromoForm({ promo, onVoltar, onSucesso, fundo = {} }) {
  ...
  async function onSubmit(values) {
    setErroGeral(''); setSubmitting(true)
    try {
      ...
      const resp = await fetch(`${API_URL}/api/v2/inscricoes`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload),
      })
      const data = await resp.json()

      if (resp.ok) onSucesso()
      else setErroGeral(data.erros ? data.erros.join(' • ') : (data.erro || 'Erro ao enviar. Tente novamente.'))
    } catch {
      ...
      <div className="card">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <FormField label="Telefone" required error={errors.telefone?.message}>
            <input type="tel" placeholder="(00) 00000-0000" maxLength={15} className={errors.telefone ? 'invalido' : ''} {...register('telefone', { onChange: e => { e.target.value = applyMaskTelefone(e.target.value) } })} />
          </FormField>

          <FormField label="CPF" required error={errors.cpf?.message}>
            <input type="text" placeholder="000.000.000-00" maxLength={14} inputMode="numeric" className={errors.cpf ? 'invalido' : ''} {...register('cpf', { onChange: e => { e.target.value = applyMaskCpf(e.target.value) } })} />
          </FormField>

          <FormField label="Número do cupom" required hint="Encontre o número no rodapé do seu cupom fiscal." error={errors.numero_cupom?.message}>
            <input type="text" placeholder="Número impresso no cupom fiscal" inputMode="numeric" className={errors.numero_cupom ? 'invalido' : ''} {...register('numero_cupom')} />
          </FormField>
          ...
          <div className="field">
            <label>Você soube da promoção por alguma influenciadora?<span className="obrigatorio"> *</span></label>
            <div className={`radio-group${errors.comprou_influencer ? ' invalido' : ''}`}>
              <label className="radio-option"><input type="radio" value="sim" {...register('comprou_influencer')} />Sim</label>
              <label className="radio-option"><input type="radio" value="nao" {...register('comprou_influencer')} />Não</label>
            </div>
            {errors.comprou_influencer && <span className="erro-campo" style={{ display: 'block' }}>{errors.comprou_influencer.message}</span>}
          </div>

          {comprouInfluencer === 'sim' && (
            <FormField label="Qual influenciadora?" required error={errors.influencer_nome?.message}>
              <input type="text" placeholder="Nome ou @ da influenciadora" className={errors.influencer_nome ? 'invalido' : ''} {...register('influencer_nome')} />
            </FormField>
          )}
          ...
          <div className={`field-lgpd${errors.lgpd ? ' lgpd-invalida' : ''}`}>
            <label className="lgpd-label">
              ...
              <span>Autorizo o Supermercados Popular a utilizar meus dados pessoais exclusivamente para fins de participação nesta promoção, conforme a <strong>Lei 13.709/2018 (LGPD)</strong>. Consulte nossa <Link to="/privacidade" target="_blank" style={{ color: '#FAC21E' }}>Política de Privacidade</Link>.</span>
            </label>
            {errors.lgpd && <span className="erro-campo" style={{ display: 'block', marginTop: 8 }}>{errors.lgpd.message}</span>}
          </div>

          <div className={`field-lgpd${errors.regulamento ? ' lgpd-invalida' : ''}`}>
            <label className="lgpd-label">
              <input type="checkbox" {...register('regulamento')} />
              <span>Declaro que li e concordo com o <strong>regulamento desta promoção</strong> do Supermercados Popular.</span>
            </label>
            {errors.regulamento && <span className="erro-campo" style={{ display: 'block', marginTop: 8 }}>{errors.regulamento.message}</span>}
          </div>
          ...
          <button type="button" onClick={onVoltar} className="btn-ghost">← Voltar</button>

          <p className="recaptcha-note">Protegido por reCAPTCHA &nbsp;·&nbsp; Seus dados estão seguros</p>
        </form>
      </div>
